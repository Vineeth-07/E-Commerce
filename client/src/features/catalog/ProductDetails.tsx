...
import { buildProductImagePath, formatCurrency, recordRecentlyViewedProduct } from "../../app/util/util";
...
export default function ProductDetails(){
    ...
    useEffect(() => {
        ...
        id && agent.Store.details(parseInt(id))       
            .then(response => {
                setProduct(response);
                recordRecentlyViewedProduct(response.id);
            })
    ...
    return (
        <Grid container spacing={6}>
            <Grid item xs={6}>
                <img src={buildProductImagePath(product)} alt={product.name} style={{ width: '100%' }}/>
            </Grid>
            <Grid item xs={6}>
                <Typography variant='h3'>{product.name}</Typography>
                <Divider sx={{ mb:2 }}/>
                <Typography gutterBottom color='secondary' variant="h4">{formatCurrency(product.price)}</Typography>
                <TableContainer>
                    <Table>
                        <TableBody>
                            <TableRow>
